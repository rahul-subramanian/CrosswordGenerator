...
# why storing the recursive backtrack soln in 'result' and not returning directly?
# bc it has to try different route and not just return None in the first failure.
# ie, the current assignment may LEAD to failure in future, so we must COME BACK and TRY DIFF VALUE before we return an assignment
# ie, before we RETURN an assignment (result), we are making sure it does not LEAD to failure in the future.
